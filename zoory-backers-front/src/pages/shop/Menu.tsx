    ...
    return (
        <div className="mt-10" >
            ...
            <div className="flex justify-center my-8">
                {
                    Array.from({length:Math.ceil(filterdItems.length / itemsPerPage)}).map((_,index) =>(
                        <button
                            key={index + 1}
                            onClick={() => paginate(index + 1)}
                            className={`mx-1 px-3 py-1 rounded-full ${currentPage === index +1 ? "bg-[#FF9800] text-white":"bg-gray-200"}`}
                        > {index + 1}  </button>
                    ) )
                }
            </div>
